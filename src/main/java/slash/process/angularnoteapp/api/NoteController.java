...
import slash.process.angularnoteapp.dbmanagement.NoteRepository;
...
public class NoteController {
    ...
    public List<NoteViewModel> all() {
        ...
        List<NoteViewModel> notesViewModel = new ArrayList<>();
        notes.forEach(note -> {notesViewModel.add(note.noteModel());});
...
//        var noteEntity = this.mapper.convertToNoteEntity(noteCreateViewModel);
        ...
        this.noteRepository.save(noteCreateViewModel.modelToNote(notebook));

        return noteCreateViewModel.modelToNote(notebook);
    }
